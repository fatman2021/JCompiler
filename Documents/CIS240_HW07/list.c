...
list_elt *addNew (list_elt *first_elt, char *name, int number)
{
  ...
  elt = malloc (sizeof(*elt));  //elt is pointer to list_elt
  ...
  // The create new element part:
  // Copy the name (which is a pointer)
  strcpy (elt->name, name);
  elt->number = number;
  
  // The make the pointers part:
  elt->prev = NULL;
  elt->next = first_elt;
...
list_elt *delete (list_elt *first_elt, list_elt *elt)
{
  ...
  if (elt == first_elt)
    return next;  // Deleted first_elt so now next is new first_elt
  else
    return first_elt;
}
