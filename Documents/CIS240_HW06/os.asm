...
TRAP_PUTS
	...
	BRz END_PUTS 	; End if character is 0
...
;;; Clear some space
	...
	CONST R2, #7 	; Initialize col index
...
SPRITE_INNER_LOOP
	LC R4, OS_VIDEO_NUM_COLS
	ADD R6, R0, R2
	CMP R6, R4  	; Check if current col reaches number of cols
	BRzp SPRITE_END_INNER_LOOP
	CMPI R2, #0 	; Check if col index reaches zero
	BRn SPRITE_END_INNER_LOOP
	CMPI R6, #0 	; Check if current col is less than zero
	BRn SPRITE_END_INNER_LOOP
;;; Have R6, R7. R5 is word, R4 is num cols
	AND R6, R5, #1  ; And lsb of word with 1
	BRz SPRITE_ADJUST_COL
	MUL R6, R1, R4  ; Multiply current row by number of columns
	ADD R6, R6, R0  ; Add current col to this result
	ADD R6, R6, R2  ; Add col index because going backwards
	LEA R4, OS_VIDEO_MEM
...
SPRITE_ADJUST_COL
	ADD R2, R2, #-1  ; Decrement col index
